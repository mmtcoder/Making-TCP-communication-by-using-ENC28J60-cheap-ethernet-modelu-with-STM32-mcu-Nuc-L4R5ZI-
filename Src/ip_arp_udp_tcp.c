...
static uint16_t ipIdntNumber = 200;
...
void init_ip_arp_udp_tcp(uint8_t *mymac,uint8_t *myip,uint8_t * destMac, uint8_t * destIp,uint16_t port){
        uint8_t i=0;
        wwwport_h=(port>>8)&0xff;
        wwwport_l=(port&0xff);
        while(i<4){
                myipaddr[i]=myip[i];
                destIpaddr[i] = destIp[i];
                i++;
        }
        i=0;
        while(i<6){
                mymacaddr[i]=mymac[i];
                destMACaddr[i] = destMac[i];
                i++;
        }
}
...
uint8_t eth_type_is_arp_and_my_ip(uint8_t *buf,uint16_t len){
        ...
        while(i<4){
                if(buf[ETH_ARP_DST_IP_P+i] != myipaddr[i]){
                        return(0);
                }
                i++;
        }
...
{
        uint8_t i=0;
        uint8_t destMac[6] = {72,231,218,83,177,57};

        //copy the destination mac from the source and fill my mac into src
        while(i<6){
                buf[ETH_DST_MAC +i]= destMac[i];//buf[ETH_SRC_MAC +i];
                buf[ETH_SRC_MAC +i]=mymacaddr[i];
                i++;
        }
        buf[12] = 0x08 ; //IPv4 protocol type
}
...
void fill_ip_hdr_checksum(uint8_t *buf)
{
        ...
        buf[IP_P] = 0x45; //Version 4bit | Header length 4 bit
        buf[IP_CHECKSUM_P]=0;
        buf[IP_CHECKSUM_P+1]=0;
        buf[IP_FLAGS_P]=0x40; // don't fragment
        buf[IP_FLAGS_P+1]=0;  // fragement offset
        buf[IP_TTL_P]=0x80; // ttl
        buf[IP_PROTO_P] =0x06; //TCP number
        buf[IP_ID_H_P] = (uint8_t) ipIdntNumber >> 8;
        buf[IP_ID_L_P] = (uint8_t) ipIdntNumber;
        ipIdntNumber++;
...
void make_ip_tcp_new(uint8_t *buf, uint16_t len,uint8_t *dst_ip)
{
	...
	// set source and destination ip address
        while(i<4){
                buf[IP_DST_P+i]=destIpaddr[i];
                buf[IP_SRC_P+i]=myipaddr[i];
                i++;
        }
...
void make_arp_answer_from_request(uint8_t *buf)
{
        ...
        while(i<6){
                ...
                buf[ETH_ARP_SRC_MAC_P+i]=mymacaddr[i];
                i++;
        }
        i=0;
        while(i<4){
                buf[ETH_ARP_DST_IP_P+i]=buf[ETH_ARP_SRC_IP_P+i];
                buf[ETH_ARP_SRC_IP_P+i]=myipaddr[i];
                i++;
        }
        // eth+arp is 42 bytes:
...
uint8_t checkArpRequestIsTrue(uint8_t * sourceBuffer,uint8_t* destIpAddress)
{
	 if(sourceBuffer[ETH_TYPE_H_P] != ETHTYPE_ARP_H_V ||
			 sourceBuffer[ETH_TYPE_L_P] != ETHTYPE_ARP_L_V){
	                return(0);
	  }
	 ...
	 while(i<4)
	 {
		 if(sourceBuffer[ARP_SENDER_PROTOCOL_ADDRS + i] == destIpAddress[i] )
		 {
...
void sendArpMACRequest(uint8_t* buf, uint8_t * sourceMacAddrs,uint8_t *sourceIp4Addrs,uint8_t * targetIp4Addrs)
{
	  uint8_t i=0;
		...
		while(i<6)
		{
				...
				buf[ETH_SRC_MAC +i]=sourceMacAddrs[i];
				i++;
		}

		i=0;

		buf[ETH_TYPE_H_P] = 0x08 ; //ARP protocol type
		buf[ETH_TYPE_L_P] = 0x06 ;

		buf[ARP_HARDWARE_TYPE_ADDRS +1] = 0x01;
		buf[ARP_PROTOCOL_TYPE_ADDRS ] = 0x08;
		buf[ARP_HARDWARE_LNGTH_ADDRS] =0x06;
		buf[ARP_PROTOCOL_LNGTH_ADDRS] =0x04;
		buf[ARP_OPER_CODE_ADDRS + 1] =0x01;


		while(i<6)
		{
			buf[ARP_SENDER_HARDWARE_ADDRS +i] =sourceMacAddrs[i];
			buf[ARP_TARGET_HARDWARE_ADDRS +i] =0x00;
		...
		while(i<4)
		{
			buf[ARP_SENDER_PROTOCOL_ADDRS +i] =sourceIp4Addrs[i];
			buf[ARP_TARGET_PROTOCOL_ADDRS +i] =targetIp4Addrs[i];
			i++;
		}


		enc28j60PacketSend(ARP_HEADER_LEN+ETH_HEADER_LEN,buf);
