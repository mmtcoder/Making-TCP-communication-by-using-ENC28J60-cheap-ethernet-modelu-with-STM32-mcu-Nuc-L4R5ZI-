...
void ES_init_ip_arp_udp_tcp(uint8_t *mymac,uint8_t *myip,uint8_t * destMac, uint8_t * destIp,uint16_t port){

	init_ip_arp_udp_tcp(mymac,myip,destMac,destIp,port);
  ...
  while( !gotIp ) {
    ...
    if(dat_p==0) {
	  ...
	  if( --dhcpTries <= 0 ) 
        ...
        //TODO:   init_ip_arp_udp_tcp(mymac,myip,destMac,destIp,port);
