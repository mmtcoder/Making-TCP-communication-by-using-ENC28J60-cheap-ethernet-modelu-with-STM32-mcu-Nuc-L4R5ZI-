...
uint8_t readControlRegister(uint8_t cntrlRegAddress)
{
	//Last 3 bit must be 0 for Opcode (MSB)
	uint8_t transmitData= (0x1F & cntrlRegAddress);
	uint8_t receiveData =0;

	//HAL_GPIO_WritePin(GPIOD, GPIO_PIN_14, RESET);
	enableChip;

	HAL_SPI_Transmit(hspi, &transmitData, 1, 100);
	HAL_SPI_Receive(hspi, &receiveData, 1, 100);
...
uint16_t enc28j60PacketReceive(uint16_t maxlen, uint8_t* packet)
{
        	uint16_t rxstat;
	uint16_t len;
	epktcntValue = enc28j60Read(EPKTCNT);
